...
namespace WebSphere.Alarms
{
    public class AlarmServer : IAlarmServer
    {
        ...
        private static List<Obj> _users;
                ...
                lock (_alarms)
                {
                    lock (ConfigLocker)
                    {
                        foreach (AlarmThreadManagerConfig config in _cfgs)
                        {
                            ...
                            if (!Double.TryParse(rawVal.LastValue.Replace('.', ','), out val))
                                val = rawVal.LastValue.ToLower() == "true" ? 1 : 0;
                            logger.Logged("Info", "checck alarm [tag:" + config.Tag.TagName + ";sval:" + val + ";  STime:" + DateTime.Now + " ]...", "AlarmThreadManager", "LoadAlarmsLastStates");
        ...
        public void AddUserEvent(string userName, float value)
        {
            var dateTime = DateTime.Now;
            var dateTimeString = dateTime.ToString(DtFormat);
            var user = _users.Where(x=>x.Name== userName).FirstOrDefault();
            var event_ = context.Events.Create();

            event_.TagId = user.Id;
            event_.Value = value;
            event_.Time = dateTime;
            ...
            _events.Add(result);
            logger.Logged("Info", "add event [" + user.Id + "][" + value + "] to alarmserver...", "AlarmThreadManager", "OpenAlarm");
        ...
        public static void LoadObjSign()
        {
            lock (ConfigLocker)
            {
                ...
                _users = (from ti in context.Objects 
        ...
        public static void LoadAlarmsConfigurations()
        {
            lock (ConfigLocker)
            {
                ...
                foreach (var tagjson in _users)
                    ...
                    event_.EventMessages.Add(new EventValMessage { Value = 13, Message = "Пользователь " + tagjson.Name + " включил отопление" });
                    event_.EventMessages.Add(new EventValMessage { Value = 14, Message = "Пользователь " + tagjson.Name + " выключил отопление" });
                    event_.EventMessages.Add(new EventValMessage { Value = 15, Message = "Пользователь " + tagjson.Name + " подал команду на сброс датчиков ОПС" });
                    event_.EventMessages.Add(new EventValMessage { Value = 16, Message = "Пользователь " + tagjson.Name + " квитировал тревогу задвижки" });
                    event_.EventMessages.Add(new EventValMessage { Value = 17, Message = "Пользователь " + tagjson.Name + " квитировал тревогу газоанализатора" });
                    event_.EventMessages.Add(new EventValMessage { Value = 18, Message = "Пользователь " + tagjson.Name + " Изменил уставки датчика" });
                    _cfgsEv.Add(event_);
                    }
                    ...
                    foreach (var tagjson in taglist)
                {
                    ...
                    var tag = new TagId
                    {
                        TagName = (Convert.ToString(alarm.Connection) + "." + tagjson.Name.Replace("_unreal", "")).Replace('/', '.'),
        ...
        public List<AlarmThreadManagerOut> GetCurrentAlarms(Int32? id = null)
        {
            ...
            for (var ai = 0; ai < _alarms_.Count; ai++)
            {
                ...
                if(outAlarm.Ack  >1)
                { 
                    var user = context.Objects.FirstOrDefault(x => (x.Id == outAlarm.Ack));
                    if (user != null)
                    { 
                        outAlarm.AckedBy = user.Name;
                    }
                }
        ...
        public bool SetAlarmAck(int alarmId,string userName)
        {
            try
            {
                lock (AlarmLocker)
                {
                    ...
                    var user= context.Objects.FirstOrDefault(x => (x.Name.ToLower() == userName.ToLower() &&x.Type==9 ));
