...
function InitMnemoSVG(name) {
	...
	var realname=name;
	if (name=='UZA1')realname= 'УЗ №1';
	else if (name=='UZA12')realname= 'УЗ №12';
    var mnemoName = getElementsByAttribute('mnemoName', 'svg');
    for (var i = 0; i < mnemoName.length; i++) { 
        SetText(mnemoName[i].id, realname);
    } 
        ...
        switch (tags[i].type) {
            ...
            case "valve":
				...
				case "zd":  
                        ...
                        GM_OPC_TAGS.push(tags[i].tag + ".diVolt");
                        //GM_OPC_TAGS.push(tags[i].tag + ".Closing");  
                        //GM_OPC_TAGS.push(tags[i].tag + ".Opening"); 
                break;
            case "rk":
    ...
    for (var i = 0; i < tags.length; i++) {
        switch (tags[i].type) {
			...
			var val=GM_OPC_VALS[tags[i].tag];
                SetText(tags[i].id, SetVlvState(val)); 
                break;
            ...
            case "analog":
			try{
                SetText(tags[i].id, ProcFl(GM_OPC_VALS[tags[i].tag], 3));				
			}
			catch{
				console.log("Ошибка обновления type:default;tag:"+tags[i].tag);				
			}
                break;
            case "discrete":
...
function SetVlvState(id) {
    switch (id) {
        case "0": return "После включения";
        case "1": return "Открыта";
        case "2": return "Закрыта";
        case "3": return "Открывается";
        case "4": return "Закрывается";
        case "5": return "Между концевиками";
