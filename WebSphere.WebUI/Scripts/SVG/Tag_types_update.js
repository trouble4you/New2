function openWindowById(openId){
	Snap('#svg').node.getElementById(openId).style.display='block';
};
function closeWindowById(closeId){
	Snap('#svg').node.getElementById(openId).style.display='none';
	};
function closeWindowByClassName(className){
	
	var svg = Snap('#svg'),
	list=svg.node.getElementsByClassName(className);
	$.each(list,function(){this.style.display='none';});
};
...
function animateZD_samsyk(z, tagsname) {
                        //GM_OPC_TAGS.push(tags[i].tag + ".diDist);
                        //GM_OPC_TAGS.push(tags[i].tag + ".diFault");
                        //GM_OPC_TAGS.push(tags[i].tag + ".diOpen");
                        //GM_OPC_TAGS.push(tags[i].tag + ".diClose");  
                        //GM_OPC_TAGS.push(tags[i].tag + ".doOpen");
                        //GM_OPC_TAGS.push(tags[i].tag + ".doClose");
    var zd = '#' + z;  
        var controlremote = ParseToBool(GM_OPC_VALS[tagsname + ".diDist"]);
        var controllocal = null;
							if(controlremote==true)	controllocal=false;
							else if (controlremote==false)controllocal=true; 
		
        var alarm = ParseToBool(GM_OPC_VALS[tagsname + ".diFault"]);
        var power = ParseToBool(GM_OPC_VALS[tagsname + ".diVolt"]);
        var isclosed = ParseToBool(GM_OPC_VALS[tagsname + ".diClose"]);
        var closing = ParseToBool(GM_OPC_VALS[tagsname + ".doClose"]);
        var isopen = ParseToBool(GM_OPC_VALS[tagsname + ".diOpen"]);
        var opening = ParseToBool(GM_OPC_VALS[tagsname + ".doOpen"]);
